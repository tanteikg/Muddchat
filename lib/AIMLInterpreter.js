...
var printDom = function(childnode,space) {
  for (var i=0;i<childnode.length; i++)
  {
    if (childnode[i].name == undefined)
	console.log(space,"text [",childnode[i].text,"]");
    else
      console.log(space,i,"- name <",childnode[i].name,">");
    if (childnode[i].hasOwnProperty('children'))
      printDom(childnode[i].children,space+" ");
  }
...
// modification by ttg to check for last portion of string instead of whole string
//                if(categoryChildNodes[i].children[0].text != previousAnswer){
		if (previousAnswer.substr(previousAnswer.length -categoryChildNodes[i].children[0].text.length, categoryChildNodes[i].children[0].text.length) == categoryChildNodes[i].children[0].text){
                    return true;
                }
                else{
                    return false;
...
// ttg - to allow multiple lines in template
//		return findFinalTextInTemplateNode(childNodesOfTemplate[i].children);

                text = findFinalTextInTemplateNode(childNodesOfTemplate[i].children);

		continue;
            }
            else if(childNodesOfTemplate[i].name === 'condition'){
                text = text + resolveSpecialNodes(childNodesOfTemplate[i]);
		continue;
            }
// ttg - to handle think
            else if(childNodesOfTemplate[i].name === 'think'){
		var temptext = findFinalTextInTemplateNode(childNodesOfTemplate[i].children);
		continue;
            }
                ...
                text = text + findFinalTextInTemplateNode([childNodesOfTemplate[i].children[randomNumber]]);
		continue
            }
                ...
                var temptext = findCorrectCategory(referredPatternText, domCategories);
		if (temptext != undefined)
			text = text + temptext;

//  ttg - to allow multiple srai within pattern          
//		return text;
		continue;

            }
            else if(childNodesOfTemplate[i].name === 'li'){
                text = text + findFinalTextInTemplateNode(childNodesOfTemplate[i].children);
//  ttg - to allow multiple srai within pattern          
		continue;
                ...
                text = text + resolveSpecialNodes(childNodesOfTemplate[i].children);
                continue;
            }
...
//		return text;
		continue;
                ...
                text = text + resolveSpecialNodes(childNodesOfTemplate[i]);
//  ttg - to allow multiple gets within pattern          
//		return text;
		continue;
            }
            else if(childNodesOfTemplate[i].name === 'sr'){
                text = text + resolveSpecialNodes(childNodesOfTemplate[i]);
//  ttg - to allow multiple sr within pattern          
//		return text;
		continue;
            }
...
// ttg - to handle multiple sets/gets
//console.log("findfinal returning ",text);
	return text;
...
//        for(var i = 0; i < innerNodes.length; i++)
{
            ...
            if(innerNodes.name === 'bot'){
                //replace bot tags by the belonging bot attribute value
                text = text + botAttributes[innerNodes.attributes.name];
            }
            else if(innerNodes.name === 'get'){
                //replace get tag by belonging variable value
                text = text + storedVariableValues[innerNodes.attributes.name];
            }
            else if(innerNodes.name === 'set'){
                //store value of set tag text into variable (variable name = attribute of set tag)
                //replace than set tag by the text value

/*		
                if(innerNodes.children[0].text === '*'){
                    ...
                    storedVariableValues[innerNodes.attributes.name] = innerNodes.children[0].text;
                }
                text = text + innerNodes.children[0].text;
//                text = text + resolveSpecialNodes(innerNodes.children);
*/
		if (innerNodes.children[0].hasOwnProperty("text"))
		{
	                if(innerNodes.children[0].text === '*')
			{
	                    //the first set-Tag with wildCard gets the first wildCardValue, the second set-Tag with wildCard gets the second wildCardValue etc.
        	            storedVariableValues[innerNodes.attributes.name] = wildCardArray[indexOfSetTagAmountWithWildCard];
                	    indexOfSetTagAmountWithWildCard++;
			}
			else
			{
		...
		{
			storedVariableValues[innerNodes.attributes.name] = findFinalTextInTemplateNode(innerNodes.children);
		}
                text = text + storedVariableValues[innerNodes.attributes.name];

            }
            else if(innerNodes.name === 'sr'){
                var result;
            ...
            else if(innerNodes.name === 'star'){
                text = text + lastWildCardValue;
            }
            else if(innerNodes.name === 'condition') {                
                // condition tag specification: list condition tag
                if(innerNodes.attributes.name === undefined){
                    if(innerNodes.children === undefined){
                        return undefined;
                    }
                    var child;
                    for(var c in innerNodes.children){
                        child = innerNodes.children[c];
                        if(child.name === 'li'){
                ...
                else if(innerNodes.attributes.value !== undefined){         
                    if (storedVariableValues[innerNodes.attributes.name] === innerNodes.attributes.value.toUpperCase()) {
                        text = text + resolveSpecialNodes(innerNodes.children);
                    }
                }
                // condition tag specification: single name list condition tags
                else if(innerNodes.children !== undefined){
                    var child;
                    for(var c in innerNodes.children){
                        child = innerNodes.children[c];
                        if(child.name === 'li'){
                            if(child.attributes.value === undefined 
                                || storedVariableValues[innerNodes.attributes.name] === child.attributes.value.toUpperCase()){
                                return findFinalTextInTemplateNode(child.children);
                            }
                        }
                    }
            ...
            else  // ttg- we are assuming that "pattern" falls there
	    {
                //normal text (no special tag)
//		for(var i = 0; i < innerNodes.length; i++)
//		  if (innerNodes[i].name == undefined)
	                  text = text + innerNodes.text;
            }
        }
